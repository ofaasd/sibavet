<section class="content-header">
    <h1>
        <a href="#" class="tombol-home"><i class="fa fa-angle-left" style="font-size: 20pt"></i>&nbsp;Lab-Kesmavet<a><small> FORM 02 - PENOMORAN CONTOH</small>
    </h1>
    <ol class="breadcrumb">
        ...
        <div class="col-12 col-lg-12">
            <div class="box box-default">
                <div class="box-header with-border">
                    <h4 class="box-title">FORM 02 - PENOMORAN CONTOH <b>{!! $kesmavet->no_epid !!}</b></h4>
                ...
                <div class="box-body wizard-content">
                    <section>
                            ...
                            <div class="col-sm-10">
                                <select name="asal_contoh_id" id="asal_contoh_id" class="form-control col-sm-5">
                                    @foreach($kota as $id => $name)
                                        <option value="{{ $id }}" {{ ($kesmavet->asal_contoh_id == $id) ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="penerima_02" id="penerima_02" class="form-control" value="{{ $kesmavet->penerima_02 ?? '' }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="catatan_02" class="col-sm-2 col-form-label">Catatan/Saran :</label>
                            <div class="col-sm-10">
                                <textarea name="catatan_02" id="catatan_02" class="form-control" rows="2" placeholder="Catatan/Saran">{{ $kesmavet->catatan_02 ?? '' }}</textarea>
                            </div>
                        </div>
                    </section>
                </div>
                <div class="box-footer flexbox">                     
                    <div class="text-right flex-grow">
                        ...
                        <a href="{!! URL::to('/lab/kesmavet/'.$kesmavet->id.'/cetak02') !!}" id="tombol-cetak" target="_blank" class="btn btn-primary">Cetak</a>
...
<script>
    ...
    $('.tombol-kembali').click(function(e){
        $preloader.fadeIn();
        gotoUrl('/lab/kesmavet/{!! $kesmavet->id !!}/form01');
    });
    ...
    $('#tombol-form03').click(function(e){
        ...
        store(function(result) {
            if(result==1){
                gotoUrl('/lab/kesmavet/{!! $kesmavet->id !!}/form03');
                $preloader.fadeOut();
            }else{
                $preloader.fadeOut();
            }
