<section class="content-header">
    <h1>
        <a href="#" class="tombol-home"><i class="fa fa-angle-left" style="font-size: 20pt"></i>&nbsp;Lab-Keswan<a><small> FORM 03 - SURAT PENGANTAR PENGIRIMAN CONTOH KE LABORATORIUM</small>
    </h1>
    <ol class="breadcrumb">
        ...
        <div class="col-12 col-lg-12">
            <div class="box box-default">
                <div class="box-header with-border">
                    <h4 class="box-title">FORM 03 - SURAT PENGANTAR PENGIRIMAN CONTOH KE LABORATORIUM <b>{!! $keswan->no_epid !!}</b></h4>
                ...
                <div class="box-body wizard-content">
                    <section>
                            ...
                            <div class="col-sm-10">
                                <select name="seksi_laboratorium_id" id="seksi_laboratorium_id" class="form-control col-sm-5">
                                    @foreach($seksiLaboratorium as $id => $name)
                                        <option value="{{ $id }}" {{ ($keswan->seksi_laboratorium_id == $id) ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-10">
                                <select name="manajer_teknis" id="manajer_teknis" class="form-control col-sm-5">
                                    @foreach($pegawai as $id => $name)
                                        <option value="{{ $id }}" {{ ($keswan->manajer_teknis == $id) ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <textarea name="catatan_03" id="catatan_03" class="form-control" rows="2" placeholder="Catatan/Saran">{{ $keswan->catatan_03 }}</textarea>
                            </div>
                        </div>
                    </section>
                </div>
                <div class="box-footer flexbox">                     
                    <div class="text-right flex-grow">
                        ...
                        <a href="{!! URL::to('/lab/keswan/'.$keswan->id.'/cetak03') !!}" id="tombol-cetak" target="_blank" class="btn btn-primary">Cetak</a>
...
<script>
    ...
    $('.tombol-kembali').click(function(e){
        $preloader.fadeIn();
        gotoUrl('/lab/keswan/{!! $keswan->id !!}/form02');
    });
